...
- In a design or grilling session, the deliverable is written decisions - issues, ADRs, glossary terms - not working code. When a change surfaces mid-session, propose it with a recommendation and ask whether to implement it or file it; default to filing. This is about code: applying a settings change that gates other work is welcome when asked for directly.
...
- When multiple open questions accumulate (e.g. after a code review or multi-part investigation), ask them one at a time, not bundled together. Wait for an answer before asking the next.
